...
#import "RSReportView.h"
#import "RSGrid.h"
#import "RSLabel.h"
#import "RSStyle.h"
#import "NSIndexPath+RSReport.h"

typedef NS_ENUM(NSUInteger, RSPart) {
    RSPartTopLeft,
    RSPartTopRight,
    RSPartBottomLeft,
    RSPartBottomRight,
...
@interface RSReportView () <UIScrollViewDelegate> {
...
@implementation RSReportView

- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        [self p_setupViews];
        [self p_setupGestureRecognizers];
        _style = [RSStyle defaultStyle];
    }
    return self;
}
...
- (void)p_loadTopLeft {
    ...
    while (row < _numberOfHeadRows) {
        ...
        RSGrid *grid = [_datasource reportView:self gridAtIndexPath:indexPath];
        if (!grid) {
            row ++;
            continue;
        }
        ...
        RSLabel *label;
        [self p_getLabel:&label forGrid:grid inPart:RSPartTopLeft];
...
- (void)p_loadTopRight {
    ...
    while (row < _numberOfHeadRows) {
        ...
        while (col < _numberOfCols) {
            ...
            grid.indexPath = indexPath;
            RSLabel *label;
            [self p_getLabel:&label forGrid:grid inPart:RSPartTopRight];
            label.indexPath = indexPath;
...
- (void)p_loadBottomLeft {
    ...
    while (row < _numberOfRows) {
        ...
        grid.indexPath = indexPath;
        RSLabel *label;
        [self p_getLabel:&label forGrid:grid inPart:RSPartBottomLeft];
        label.indexPath = indexPath;
...
- (void)p_loadBottomRight {
    ...
    while (row < _numberOfRows) {
        ...
        while (col < _numberOfCols) {
            ...
            grid.indexPath = indexPath;
            RSLabel *label;
            [self p_getLabel:&label forGrid:grid inPart:RSPartBottomRight];
            label.indexPath = indexPath;
...
- (void)p_getLabel:(RSLabel **)label forGrid:(RSGrid *)grid inPart:(RSPart)part {
    RSLabel *theLabel = [RSLabel new];
    switch (part) {
        case RSPartTopLeft:
        case RSPartTopRight:
        {
        ...
        case RSPartBottomLeft:
        case RSPartBottomRight:
        {
...
- (NSArray *)p_suitableHeights {
    ...
    for (UIView *contentView in @[_topLeftView, _topRightView, _bottomLeftView, _bottomRightView]) {
        for (RSLabel *label in contentView.subviews) {
            if (label.rowspan > 1) {
                continue;
            }
...
- (RSLabel *)p_touchedLabelWithGesture:(UIGestureRecognizer *)gestureRecognizer {
    ...
    for (UIView *subview in inView.subviews) {
        if ([subview isKindOfClass:[RSLabel class]]) {
            CGRect rect = [subview convertRect:subview.bounds toView:self];
            if (CGRectContainsPoint(rect, point)) {
                label = (RSLabel *)subview;
                break;
            }
        }
    }
...
- (void)p_handleLongPressGR:(UILongPressGestureRecognizer *)longPressGR {
    ...
    if (_delegateFlags.didLongPressLabel) {
        RSLabel *label = [self p_touchedLabelWithGesture:longPressGR];
        if (label) {
...
- (void)p_handleTapGR:(UILongPressGestureRecognizer *)tapGR {
    if (_delegateFlags.didTapLabel) {
        RSLabel *label = [self p_touchedLabelWithGesture:tapGR];
        if (label) {
...
- (void)sortByCol:(NSInteger)col order:(RSOrder)order {
    ...
    if (_sortedOrder != RSOrderedNature) {
        RSLabel *label;
        if (_sortedCol == 0) {
    ...
    for (NSNumber *index in indexes) {
        ...
        RSLabel *label = _bottomLeftView.subviews[index.integerValue - 1];
        CGRect rect = label.frame;
        rect.origin.y = y;
        label.frame = rect;
        y += CGRectGetHeight(rect) + _style.spacing;
    }
    for (RSLabel *label in _bottomRightView.subviews) {
        CGRect rect = label.frame;
